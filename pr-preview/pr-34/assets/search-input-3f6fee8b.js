import{r as a,c as B,d as M,e as N,q as w,a as o,F as h,f as U,g as q}from"./header-76e2ce0d.js";import{i as g}from"./vendor-790bbfe5.js";function $(e){return e.charAt(0).toUpperCase()+e.slice(1)}function H(e){const t=e.map(({filter:s,name:i,imgURL:r})=>`<li class="category-card fade-item" data-filter="${s}" data-name="${i}">
            <img
              src="${r}"
              alt="${i}"
            ...
            <div class="category-card-content">
              <h3 class="category-card-title">${$(s)}</h3>
              <p class="category-card-text">${$(i)}</p> 
            </div>
        </li>`).join("");a.list.innerHTML=t}function C(e){return`<svg
              ...
              >
                <path
                  ...
                  fill="#242424"
                />
              </svg>`}function l({pageId:e,content:t,liClassNames:s}){const i=document.createElement("li"),r=document.createElement("a");return r.classList.add("pagination-link"),r.onclick=n=>{n.preventDefault()},r.href="#",r.innerHTML=t,i.classList.add(...s.filter(Boolean)),i.dataset.id=e,i.appendChild(r),i}function y(e,t){const s=document.createDocumentFragment(),i=Array(e).fill(0);if(e>9){const r=l({pageId:1,content:`${C("first")}${C("first")}`,liClassNames:["pagination-arrow","go-to-first",t===1?"disabled":""]});s.appendChild(r);const n=l({pageId:t-1<1?1:t-1,content:C(),liClassNames:["pagination-arrow","go-to-prev",t===1?"disabled":""]});s.appendChild(n);let c=t-1<1?1:t-1,d=t+1>e?e:t+1;if(d-c<2&&(c===1?d=c+2:c=d-2),c!==1){const m=l({pageId:-1,content:"...",liClassNames:["pagination-item","three-dots"]});s.appendChild(m)}if(i.slice(c-1,d).map((m,v)=>{const F=l({pageId:c+v,content:c+v,liClassNames:["pagination-item",t===c+v?"active":""]});s.appendChild(F)}),d!==e){const m=l({pageId:-1,content:"...",liClassNames:["pagination-item","three-dots"]});s.appendChild(m)}const u=l({pageId:t+1>e?e:t+1,content:x(),liClassNames:["pagination-arrow","go-to-next",t===e?"disabled":""]});s.appendChild(u);const T=l({pageId:e,content:`${x("last")}${x("last")}`,liClassNames:["pagination-arrow","go-to-last",t===e?"disabled":""]});s.appendChild(T)}else i.map((r,n)=>{const c=l({pageId:n+1,content:n+1,liClassNames:["pagination-item",t===n+1?"active":""]});s.appendChild(c)});a.pagination.innerHTML="",a.pagination.appendChild(s)}const A=e=>e&&e.charAt(0).toUpperCase()+e.slice(1);function D(e){const t=e.map(({_id:s,name:i,rating:r,bodyPart:n,burnedCalories:c,target:d,time:u})=>`<li class="exercise-card fade-item">
          ...
          <div class="exercise-card-bottom">
            <div class="exercise-card-title">
              ...
              <p class="exercise-card-title-text">${A(i)}</p>
            </div>
        ...
        </li>`).join("");a.list.innerHTML=t}function k(){const e=document.getElementsByClassName("fade-item");for(let s=0;s<e.length;++s)t(e[s],s*400);function t(s,i){setTimeout(()=>{s.classList.add("fadein")},i)}}function b(){B().then(e=>{e.totalPages,H(e.results),y(e.totalPages,Number(e.page)),k()}).catch(e=>{g.error({message:`${e}`})})}function f(){M().then(e=>{e.results.length||g.warning({message:"Sorry, this request did not match. Try changing the request parameters."}),D(e.results),y(e.totalPages,Number(e.page)),k()}).catch(e=>{g.error({message:`${e}`})})}const L={target:"Target",bodyPart:"Body Part",equipment:"Equipment",popularity:"Popular"};function O(e){let t="";return Object.keys(L).forEach(s=>{e.hasOwnProperty(s)&&(t+=`
    ...
    `;let r="";for(let n=0;n<5;n++)e>=1?r+=t:e>0?r+=i(e*100):r+=s,e--;return r}function j(e){const t=O(e),s=P(e.rating);return`
  ...
  `}function G(e){z(e).then(()=>{a.exercisePopUpBackdrop.classList.add("is-open"),a.body.classList.add("overflow-hidden"),R()}).catch(t=>{g.error({message:`${t}`})})}function I(){a.exercisePopUpBackdrop.classList.remove("is-open"),a.body.classList.remove("overflow-hidden"),Z()}function z(e){return new Promise((t,s)=>{N(e).then(i=>{a.exercisePopUpContent.innerHTML=j(i),t()}).catch(i=>{s(i)})})}function R(){document.addEventListener("click",E)}function Z(){document.removeEventListener("click",E)}function E(e){const t=a.exercisePopUpContent;t&&!t.contains(e.target)&&I()}a.exercisePopUpCloseBtn.addEventListener("click",I);function W(e){const t=e.target,s=t.closest("li.pagination-item:not(.active):not(.three-dots)")??t.closest("li.pagination-arrow:not(.disabled)");if(!s)return;const i=Number(s.dataset.id);w.set("page",i),o.set("page",i),a.exerciseHeaderSpan.dataset.exercise?f():b()}function J(e){const s=e.target.closest("li[data-filter]");if(!s)return;document.querySelectorAll(".js-filters .active").forEach(c=>c.classList.remove("active")),s.classList.add("active"),a.searchForm.reset(),a.searchForm.classList.add("visually-hidden");const r=s.dataset.filter,n=Object.keys(h).find(c=>h[c]===r);n&&(w.set("filter",h[n]),w.set("page",1),o.delete("muscles"),o.delete("bodypart"),o.delete("equipment"),o.set("page",1),a.list.classList.replace("exercise-list","filtered-list"),b()),U()}function Q(e){const s=e.target.closest("li.category-card");if(!s)return;let i=s.dataset.filter.toString().toLowerCase();i==="body parts"&&(i="bodypart");const r=s.dataset.name;q(r),o.set(i,r),o.set("page",1),a.list.classList.replace("filtered-list","exercise-list"),a.searchForm.classList.remove("visually-hidden"),a.list.innerHTML="",f()}function S(e){e.preventDefault();const t=a.searchInput.value.trim();o.set("keyword",t),f()}function V(e){const t=e.target.closest(".exercise-start-btn");if(!t)return;const s=t.getAttribute("data-exercise-id");s&&G(s)}a.searchInputCrossIcon.addEventListener("click",()=>{o.set("page",1),o.delete("keyword"),a.searchForm.reset(),f()});a.searchInputSearchIcon.addEventListener("click",S);a.searchForm.addEventListener("submit",S);export{W as a,Q as b,V as c,J as o,b as r};
//# sourceMappingURL=search-input-3f6fee8b.js.map
