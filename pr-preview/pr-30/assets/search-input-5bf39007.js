import{r as i,b as L,c as E,d as P,q as f,a as n,F as u,e as w,f as b}from"./header-0256d207.js";import{i as d}from"./vendor-790bbfe5.js";function C(e){const t=e.map(({filter:s,name:r,imgURL:a})=>`<li class="filtered-item" data-filter="${s}" data-name="${r}">
            <img
              src="${a}"
              alt="${r}"
              class="filter-img"
        ...
        </li>`).join("");i.list.innerHTML=t}function m(e){let t="";for(let s=1;s<=e;s++)t+=`<li class="pagination-item" data-id="${s}"><a href="#" onclick="event.preventDefault();" class="pagination-link">${s}</a></li>`;i.pagination.innerHTML=t}const S=e=>e&&e.charAt(0).toUpperCase()+e.slice(1);function I(e){const t=e.map(({_id:s,name:r,rating:a,bodyPart:c,burnedCalories:l,target:k,time:y})=>`<li class="exercise-card">
          ...
          <div class="exercise-card-bottom">
            ...
            <div class="exercise-card-info"> 
              <div class="exercise-card-info-element">
                ...
                <div class="exercise-card-info-element-content-target-no-overflow">${l} / ${y} min</div>
              </div>
              <div class="exercise-card-info-element">
  ...
  `}function T(e){U(e).then(()=>{i.exercisePopUpBackdrop.classList.add("is-open"),q()}).catch(t=>{d.error({message:`${t}`})})}function x(){i.exercisePopUpBackdrop.classList.remove("is-open"),H()}function U(e){return new Promise((t,s)=>{P(e).then(r=>{i.exercisePopUpContent.innerHTML=B(r),t()}).catch(r=>{s(r)})})}function q(){document.addEventListener("click",h)}function H(){document.removeEventListener("click",h)}function h(e){const t=i.exercisePopUpContent;t&&!t.contains(e.target)&&x()}i.exercisePopUpCloseBtn.addEventListener("click",x);function N(e){const s=e.target.closest("li.pagination-item");if(!s)return;const r=Number(s.dataset.id);f.set("page",r),n.set("page",r),i.exerciseHeaderSpan.dataset.exercise?p():v()}function O(e){const s=e.target.closest("li[data-filter]");if(!s)return;document.querySelectorAll(".js-filters .active").forEach(l=>l.classList.remove("active")),s.classList.add("active"),i.searchForm.reset(),i.searchForm.classList.add("visually-hidden");const a=s.dataset.filter,c=Object.keys(u).find(l=>u[l]===a);c&&(f.set("filter",u[c]),f.set("page",1),n.delete("muscles"),n.delete("bodypart"),n.delete("equipment"),n.set("page",1),i.list.classList.replace("exercise-list","filtered-list"),v()),w()}function j(e){const s=e.target.closest("li.filtered-item");if(!s)return;let r=s.dataset.filter.toString().toLowerCase();r==="body parts"&&(r="bodypart");const a=s.dataset.name;b(a),n.set(r,a),n.set("page",1),i.list.classList.replace("filtered-list","exercise-list"),i.searchForm.classList.remove("visually-hidden"),i.list.innerHTML="",p()}function $(e){e.preventDefault();const t=i.searchInput.value.trim();n.set("keyword",t),p()}function G(e){const t=e.target.closest(".exercise-start-btn");if(!t)return;const s=t.getAttribute("data-exercise-id");s&&(console.log("Exercise ID:",s),T(s))}i.searchInputCrossIcon.addEventListener("click",()=>{n.set("page",1),n.delete("keyword"),i.searchForm.reset(),p()});i.searchInputSearchIcon.addEventListener("click",$);i.searchForm.addEventListener("submit",$);export{N as a,j as b,G as c,O as o,v as r};
//# sourceMappingURL=search-input-5bf39007.js.map
