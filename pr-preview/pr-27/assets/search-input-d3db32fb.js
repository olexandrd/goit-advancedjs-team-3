import{r as a,c as T,d as F,e as M,q as w,a as o,F as h,f as B,g as N}from"./header-62ddc169.js";import{i as g}from"./vendor-7b59b48c.js";function U(e){const t=e.map(({filter:s,name:i,imgURL:r})=>`<li class="filtered-item" data-filter="${s}" data-name="${i}">
            <img
              src="${r}"
              alt="${i}"
              class="filter-img"
            ...
            <p class="filtered-text">${i}</p>
              ...
              >
                <path
                  ...
                  fill="#242424"
                />
              </svg>`}function l({pageId:e,content:t,liClassNames:s}){const i=document.createElement("li"),r=document.createElement("a");return r.classList.add("pagination-link"),r.onclick=n=>{n.preventDefault()},r.href="#",r.innerHTML=t,i.classList.add(...s.filter(Boolean)),i.dataset.id=e,i.appendChild(r),i}function L(e,t){const s=document.createDocumentFragment(),i=Array(e).fill(0);if(e>9){const r=l({pageId:1,content:`${C("first")}${C("first")}`,liClassNames:["pagination-arrow","go-to-first",t===1?"disabled":""]});s.appendChild(r);const n=l({pageId:t-1<1?1:t-1,content:C(),liClassNames:["pagination-arrow","go-to-prev",t===1?"disabled":""]});s.appendChild(n);let c=t-1<1?1:t-1,d=t+1>e?e:t+1;if(d-c<2&&(c===1?d=c+2:c=d-2),c!==1){const m=l({pageId:-1,content:"...",liClassNames:["pagination-item","three-dots"]});s.appendChild(m)}if(i.slice(c-1,d).map((m,v)=>{const S=l({pageId:c+v,content:c+v,liClassNames:["pagination-item",t===c+v?"active":""]});s.appendChild(S)}),d!==e){const m=l({pageId:-1,content:"...",liClassNames:["pagination-item","three-dots"]});s.appendChild(m)}const u=l({pageId:t+1>e?e:t+1,content:x(),liClassNames:["pagination-arrow","go-to-next",t===e?"disabled":""]});s.appendChild(u);const I=l({pageId:e,content:`${x("last")}${x("last")}`,liClassNames:["pagination-arrow","go-to-last",t===e?"disabled":""]});s.appendChild(I)}else i.map((r,n)=>{const c=l({pageId:n+1,content:n+1,liClassNames:["pagination-item",t===n+1?"active":""]});s.appendChild(c)});a.pagination.innerHTML="",a.pagination.appendChild(s)}const q=e=>e&&e.charAt(0).toUpperCase()+e.slice(1);function H(e){const t=e.map(({_id:s,name:i,rating:r,bodyPart:n,burnedCalories:c,target:d,time:u})=>`<li class="exercise-card">
    ...
    `;let r="";for(let n=0;n<5;n++)e>=1?r+=t:e>0?r+=i(e*100):r+=s,e--;return r}function O(e){const t=A(e),s=D(e.rating);return`
