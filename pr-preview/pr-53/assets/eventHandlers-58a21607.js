import{i as p}from"./vendor-7b59b48c.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))r(i);new MutationObserver(i=>{for(const n of i)if(n.type==="childList")for(const o of n.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&r(o)}).observe(document,{childList:!0,subtree:!0});function s(i){const n={};return i.integrity&&(n.integrity=i.integrity),i.referrerPolicy&&(n.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?n.credentials="include":i.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function r(i){if(i.ep)return;i.ep=!0;const n=s(i);fetch(i.href,n)}})();const f="https://your-energy.b.goit.study/api/",a={list:document.querySelector(".js-filtered-list"),filter:document.querySelector(".js-filters"),pagination:document.querySelector(".js-pagination"),exerciseHeaderSpan:document.querySelector(".js-exercise-header-span"),exerciseHeader:document.querySelector(".js-exercise-header"),searchForm:document.querySelector(".search-form"),searchInput:document.querySelector(".search-input"),searchInputCrossIcon:document.querySelector(".search-input-icon.cross-icon"),searchInputSearchIcon:document.querySelector(".search-input-icon.search-icon"),exercisePopUpBackdrop:document.querySelector(".exercise-pop-up-backdrop"),exercisePopUpContent:document.querySelector(".exercise-pop-up-content"),exercisePopUpCloseBtn:document.querySelector(".exercise-pop-up-close-btn"),exercisesContainer:document.querySelector(".exercises-container"),body:document.querySelector("body"),scrollUpButton:document.querySelector(".scroll-up-button"),mainSection:document.querySelector(".main-section")},h={MUSCLES:"Muscles",BODY_PARTS:"Body parts",EQUIPMENT:"Equipment"},x=new URLSearchParams({filter:h.MUSCLES,page:1,limit:window.matchMedia("(min-width: 768px)").matches?12:9}),c=new URLSearchParams({page:1,limit:window.matchMedia("(min-width: 768px)").matches?10:8}),y={method:"GET",headers:{"Content-Type":"application/json"}};async function H(){const e=await fetch(`${f}exercises?${c}`,y);if(!e.ok)throw new Error(`Ooops, status code: ${e.status}`);return await e.json()}async function A(e){const t=await fetch(`${f}exercises/${e}`,y);if(!t.ok)throw new Error(`Ooops, status code: ${t.status}`);return await t.json()}async function R(){const e=await fetch(`${f}filters?${x}`,y);if(!e.ok)throw new Error(`Ooops, status code: ${e.status}`);return await e.json()}async function se(){const e=await fetch(`${f}quote`,y);if(!e.ok)throw new Error(`Ooops, status code: ${e.status}`);return await e.json()}async function ie(e){const t={method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e})},s=await fetch(`${f}subscription`,t),r=await s.json();if(!s.ok)throw new Error(r.message||"Failed to subscribe");return r}function k(e){return e.charAt(0).toUpperCase()+e.slice(1)}function G(e){const t=e.map(({filter:s,name:r,imgURL:i})=>`<li class="category-card fade-item" data-filter="${s}" data-name="${r}">
            ...
            <div class="category-card-content">
              <h3 class="category-card-title">${k(s)}</h3>
              <p class="category-card-text">${k(r)}</p> 
            </div>
              ...
              >
                <path
                  ...
                  fill="#242424"
                />
              </svg>`}function l({pageId:e,content:t,liClassNames:s}){const r=document.createElement("li"),i=document.createElement("a");return i.classList.add("pagination-link"),i.onclick=n=>{n.preventDefault()},i.href="#",i.innerHTML=t,r.classList.add(...s.filter(Boolean)),r.dataset.id=e,r.appendChild(i),r}function B(e,t){const s=document.createDocumentFragment(),r=Array(e).fill(0);if(e>9){const i=l({pageId:1,content:`${S("first")}${S("first")}`,liClassNames:["pagination-arrow","go-to-first",t===1?"disabled":""]});s.appendChild(i);const n=l({pageId:t-1<1?1:t-1,content:S(),liClassNames:["pagination-arrow","go-to-prev",t===1?"disabled":""]});s.appendChild(n);let o=t-1<1?1:t-1,d=t+1>e?e:t+1;if(d-o<2&&(o===1?d=o+2:o=d-2),o!==1){const v=l({pageId:-1,content:"...",liClassNames:["pagination-item","three-dots"]});s.appendChild(v)}if(r.slice(o-1,d).map((v,b)=>{const j=l({pageId:o+b,content:o+b,liClassNames:["pagination-item",t===o+b?"active":""]});s.appendChild(j)}),d!==e){const v=l({pageId:-1,content:"...",liClassNames:["pagination-item","three-dots"]});s.appendChild(v)}const w=l({pageId:t+1>e?e:t+1,content:L(),liClassNames:["pagination-arrow","go-to-next",t===e?"disabled":""]});s.appendChild(w);const E=l({pageId:e,content:`${L("last")}${L("last")}`,liClassNames:["pagination-arrow","go-to-last",t===e?"disabled":""]});s.appendChild(E)}else r.map((i,n)=>{const o=l({pageId:n+1,content:n+1,liClassNames:["pagination-item",t===n+1?"active":""]});s.appendChild(o)});a.pagination.innerHTML="",a.pagination.appendChild(s)}const D=e=>e&&e.charAt(0).toUpperCase()+e.slice(1);function _(e,t=!1){return e.map(({_id:s,name:r,rating:i,bodyPart:n,burnedCalories:o,target:d,time:w})=>`<li class="exercise-card fade-item">
          ...
          <div class="exercise-card-bottom">
            <div class="exercise-card-title">
              ...
              <p class="exercise-card-title-text">${D(r)}</p>
            </div>
        ...
        </li>`).join("")}function F(){const e=document.getElementsByClassName("fade-item");for(let s=0;s<e.length;++s)t(e[s],s*300);function t(s,r){setTimeout(()=>{s.classList.add("fadein")},r)}}function N(){R().then(e=>{e.totalPages,G(e.results),B(e.totalPages,Number(e.page)),F()}).catch(e=>{p.error({message:`${e}`})})}function $(){H().then(e=>{e.results.length||p.warning({message:"Sorry, this request did not match. Try changing the request parameters."}),a.list.innerHTML=_(e.results),B(e.totalPages,Number(e.page)),F()}).catch(e=>{p.error({message:`${e}`})})}const I=document.querySelector(".header"),q=document.querySelector("[data-home]"),T=document.querySelector("[data-favorites]");let g=!1;window.addEventListener("scroll",()=>{const e=window.scrollY||document.documentElement.scrollTop,t=document.querySelector(".scroll-up-button-container");e>=2&&!g?(I.classList.add("header-scrolled"),t.classList.remove("visually-hidden"),g=!0):e<2&&g&&(t.classList.add("visually-hidden"),I.classList.remove("header-scrolled"),g=!1)});(function(){window.location.href.includes("favorites.html")?(q.classList.remove("active"),T.classList.add("active")):(q.classList.add("active"),T.classList.remove("active"))})();function J(e){a.exerciseHeader.textContent="Exercises /",a.exerciseHeaderSpan.textContent=`${e}`,a.exerciseHeaderSpan.dataset.exercise=e}function z(){a.exerciseHeader.textContent="Exercises",a.exerciseHeaderSpan.textContent="",a.exerciseHeaderSpan.dataset.exercise=""}const P={target:"Target",bodyPart:"Body Part",equipment:"Equipment",popularity:"Popular"};function K(e){let t="";return Object.keys(P).forEach(s=>{e.hasOwnProperty(s)&&(t+=`
    ...
    </div>`;let i=e,n="";for(let o=0;o<5;o++)i>=1?n+=t:i>0?n+=r(i*100):n+=s,i--;return`
  ...
  `}let u=null,m=null;function U(e){W(e).then(t=>{u=null,m=null,u=document.querySelector("#add-favorites"),m=document.querySelector("#remove-favorites"),u&&(u.dataset.exercise=JSON.stringify(t),u.addEventListener("click",ee,{passive:!0})),m&&(m.dataset.exercise=JSON.stringify(t)),a.exercisePopUpCloseBtn.addEventListener("click",C,{passive:!0}),a.exercisePopUpBackdrop.addEventListener("click",O,{passive:!0}),a.exercisePopUpBackdrop.classList.add("is-open"),a.body.classList.add("overflow-hidden"),document.addEventListener("keydown",M)}).catch(t=>{p.error({message:`${t}`})})}function C(){a.exercisePopUpBackdrop.classList.remove("is-open"),a.body.classList.remove("overflow-hidden"),u=null,m=null,a.exercisePopUpBackdrop.removeEventListener("click",O),a.exercisePopUpCloseBtn.removeEventListener("click",C),document.removeEventListener("keydown",M)}function W(e){return new Promise((t,s)=>{A(e).then(r=>{a.exercisePopUpContent.innerHTML=Z(r),t(r)}).catch(r=>{s(r)})})}function O(e){e.target===a.exercisePopUpBackdrop&&C()}function X(e){let t=JSON.parse(localStorage.getItem("favorites"))||[];t.some(r=>r._id===e._id)?p.warning({message:"Exercise is already in favorites!"}):(t.push(e),localStorage.setItem("favorites",JSON.stringify(t)),p.success({message:"Exercise added to favorites!"}))}function ee(e){const t=JSON.parse(u.dataset.exercise);X(t),e.stopPropagation(),U(t._id)}function M(e){e.code==="Escape"&&C()}function re(e){const t=e.target,s=t.closest("li.pagination-item:not(.active):not(.three-dots)")??t.closest("li.pagination-arrow:not(.disabled)");if(!s)return;const r=Number(s.dataset.id);x.set("page",r),c.set("page",r),a.exerciseHeaderSpan.dataset.exercise?$():N(),a.mainSection.scrollIntoView()}function ae(e){const s=e.target.closest("li[data-filter]");if(!s)return;document.querySelectorAll(".js-filters .active").forEach(o=>o.classList.remove("active")),s.classList.add("active"),a.searchForm.reset(),a.searchForm.classList.add("visually-hidden");const i=s.dataset.filter,n=Object.keys(h).find(o=>h[o]===i);n&&(x.set("filter",h[n]),x.set("page",1),c.delete("muscles"),c.delete("bodypart"),c.delete("equipment"),c.set("page",1),a.list.classList.replace("exercise-list","filtered-list"),N()),z()}function ne(e){const s=e.target.closest("li.category-card");if(!s)return;let r=s.dataset.filter.toString().toLowerCase();r==="body parts"&&(r="bodypart");const i=s.dataset.name;J(i),c.set(r,i),c.set("page",1),a.list.classList.replace("filtered-list","exercise-list"),a.searchForm.classList.remove("visually-hidden"),a.list.innerHTML="",$(),a.mainSection.scrollIntoView()}function oe(e){e.preventDefault();const t=a.searchInput.value.trim();c.set("keyword",t),$()}function ce(e){const t=e.target.closest(".exercise-start-btn");if(!t)return;const s=t.getAttribute("data-exercise-id");s&&U(s)}export{c as a,N as b,re as c,ne as d,ce as e,_ as f,F as g,se as h,$ as i,oe as j,ae as o,x as q,a as r,ie as s};
//# sourceMappingURL=eventHandlers-58a21607.js.map
