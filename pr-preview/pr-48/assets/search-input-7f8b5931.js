import{r as a,c as N,d as T,e as M,q as C,a as o,F as u,f as P,g as U}from"./header-87cf2a2a.js";import{i as p}from"./vendor-7b59b48c.js";function w(e){return e.charAt(0).toUpperCase()+e.slice(1)}function A(e){const s=e.map(({filter:t,name:i,imgURL:n})=>`<li class="category-card fade-item" data-filter="${t}" data-name="${i}">
            <img
              src="${n}"
              alt="${i}"
  ...
  `}function G(e){J(e).then(s=>{a.exercisePopUpBackdrop.classList.add("is-open"),a.body.classList.add("overflow-hidden"),a.exercisePopupAddFavoritesBtn.dataset.exercise=JSON.stringify(s),a.exercisePopUpCloseBtn.addEventListener("click",L,{passive:!0}),a.exercisePopupAddFavoritesBtn.addEventListener("click",b,{passive:!0}),a.exercisePopUpBackdrop.addEventListener("click",E,{passive:!0})}).catch(s=>{p.error({message:`${s}`})})}function L(){a.exercisePopUpBackdrop.classList.remove("is-open"),a.body.classList.remove("overflow-hidden"),a.exercisePopupAddFavoritesBtn.removeEventListener("click",b),a.exercisePopUpBackdrop.removeEventListener("click",E),a.exercisePopUpCloseBtn.removeEventListener("click",L)}function J(e){return new Promise((s,t)=>{M(e).then(i=>{a.exercisePopUpContent.innerHTML=j(i),s(i)}).catch(i=>{t(i)})})}function E(e){e.target===a.exercisePopUpBackdrop&&L()}function R(e){let s=JSON.parse(localStorage.getItem("favorites"))||[];s.some(i=>i._id===e._id)?p.warning({message:"Exercise is already in favorites!"}):(s.push(e),localStorage.setItem("favorites",JSON.stringify(s)),p.success({message:"Exercise added to favorites!"}))}function b(e){const s=JSON.parse(a.exercisePopupAddFavoritesBtn.dataset.exercise);R(s),e.stopPropagation()}function V(e){const s=e.target,t=s.closest("li.pagination-item:not(.active):not(.three-dots)")??s.closest("li.pagination-arrow:not(.disabled)");if(!t)return;const i=Number(t.dataset.id);C.set("page",i),o.set("page",i),a.exerciseHeaderSpan.dataset.exercise?g():I(),a.mainSection.scrollIntoView()}function Z(e){const t=e.target.closest("li[data-filter]");if(!t)return;document.querySelectorAll(".js-filters .active").forEach(r=>r.classList.remove("active")),t.classList.add("active"),a.searchForm.reset(),a.searchForm.classList.add("visually-hidden");const n=t.dataset.filter,c=Object.keys(u).find(r=>u[r]===n);c&&(C.set("filter",u[c]),C.set("page",1),o.delete("muscles"),o.delete("bodypart"),o.delete("equipment"),o.set("page",1),a.list.classList.replace("exercise-list","filtered-list"),I()),P()}function K(e){const t=e.target.closest("li.category-card");if(!t)return;let i=t.dataset.filter.toString().toLowerCase();i==="body parts"&&(i="bodypart");const n=t.dataset.name;U(n),o.set(i,n),o.set("page",1),a.list.classList.replace("filtered-list","exercise-list"),a.searchForm.classList.remove("visually-hidden"),a.list.innerHTML="",g(),a.mainSection.scrollIntoView()}function F(e){e.preventDefault();const s=a.searchInput.value.trim();o.set("keyword",s),g()}function W(e){const s=e.target.closest(".exercise-start-btn");if(!s)return;const t=s.getAttribute("data-exercise-id");t&&G(t)}a.searchInputCrossIcon.addEventListener("click",()=>{o.set("page",1),o.delete("keyword"),a.searchForm.reset(),g()});a.searchInputSearchIcon.addEventListener("click",F);a.searchForm.addEventListener("submit",F);export{V as a,K as b,W as c,Z as o,I as r};
//# sourceMappingURL=search-input-7f8b5931.js.map
