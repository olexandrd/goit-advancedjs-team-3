import{r as i,c as y,d as k,e as L,q as g,a as n,F as f,f as w,g as E}from"./header-65c596cf.js";import{i as d}from"./vendor-790bbfe5.js";function P(e){const t=e.map(({filter:s,name:r,imgURL:a})=>`<li class="filtered-item" data-filter="${s}" data-name="${r}">
            <img
              src="${a}"
              alt="${r}"
              class="filter-img"
        ...
        </li>`).join("");i.list.innerHTML=t}function v(e){let t="";for(let s=1;s<=e;s++)t+=`<li class="pagination-item" data-id="${s}"><a href="#" onclick="event.preventDefault();" class="pagination-link">${s}</a></li>`;i.pagination.innerHTML=t}const b=e=>e&&e.charAt(0).toUpperCase()+e.slice(1);function S(e){const t=e.map(({_id:s,name:r,rating:a,bodyPart:c,burnedCalories:l,target:x,time:$})=>`<li class="exercise-card">
  ...
  `}function M(e){T(e).then(()=>{i.exercisePopUpBackdrop.classList.add("is-open"),i.body.classList.add("overflow-hidden")}).catch(t=>{d.error({message:`${t}`})})}function B(){i.exercisePopUpBackdrop.classList.remove("is-open"),i.body.classList.remove("overflow-hidden")}function T(e){return new Promise((t,s)=>{L(e).then(r=>{i.exercisePopUpContent.innerHTML=F(r),t()}).catch(r=>{s(r)})})}i.exercisePopUpCloseBtn.addEventListener("click",B);function H(e){const s=e.target.closest("li.pagination-item");if(!s)return;const r=Number(s.dataset.id);g.set("page",r),n.set("page",r),i.exerciseHeaderSpan.dataset.exercise?p():m()}function A(e){const s=e.target.closest("li[data-filter]");if(!s)return;document.querySelectorAll(".js-filters .active").forEach(l=>l.classList.remove("active")),s.classList.add("active"),i.searchForm.reset(),i.searchForm.classList.add("visually-hidden");const a=s.dataset.filter,c=Object.keys(f).find(l=>f[l]===a);c&&(g.set("filter",f[c]),g.set("page",1),n.delete("muscles"),n.delete("bodypart"),n.delete("equipment"),n.set("page",1),i.list.classList.replace("exercise-list","filtered-list"),m()),w()}function D(e){const s=e.target.closest("li.filtered-item");if(!s)return;let r=s.dataset.filter.toString().toLowerCase();r==="body parts"&&(r="bodypart");const a=s.dataset.name;E(a),n.set(r,a),n.set("page",1),i.list.classList.replace("filtered-list","exercise-list"),i.searchForm.classList.remove("visually-hidden"),i.list.innerHTML="",p()}function h(e){e.preventDefault();const t=i.searchInput.value.trim();n.set("keyword",t),p()}function N(e){const t=e.target.closest(".exercise-start-btn");if(!t)return;const s=t.getAttribute("data-exercise-id");s&&(console.log("Exercise ID:",s),M(s))}i.searchInputCrossIcon.addEventListener("click",()=>{n.set("page",1),n.delete("keyword"),i.searchForm.reset(),p()});i.searchInputSearchIcon.addEventListener("click",h);i.searchForm.addEventListener("submit",h);export{H as a,D as b,N as c,A as o,m as r};
//# sourceMappingURL=search-input-aa5e902c.js.map
