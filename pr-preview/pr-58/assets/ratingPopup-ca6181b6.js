import{e as g,d as y,i as d}from"./vendor-7b59b48c.js";import{j as b,r as n,k as E}from"./eventHandlers-396a5a98.js";const a=document.querySelector(".js-menu-container"),l=document.querySelector(".js-open-menu"),S=document.querySelector(".js-close-menu"),h=document.querySelectorAll(".mobile-menu-link"),o=()=>{const e=l.getAttribute("aria-expanded")==="true"||!1;l.setAttribute("aria-expanded",!e),a.classList.toggle("is-open"),e?(g(document.body),document.removeEventListener("keydown",u),a.removeEventListener("click",m)):(y(document.body),document.addEventListener("keydown",u),a.addEventListener("click",m))},u=e=>{e.key==="Escape"&&o()},m=e=>{e.target===a&&o()};l.addEventListener("click",o);S.addEventListener("click",o);h.forEach(e=>{e.addEventListener("click",o)});window.matchMedia("(min-width: 768px)").addEventListener("change",e=>{e.matches&&(a.classList.remove("is-open"),l.setAttribute("aria-expanded",!1),g(document.body),document.removeEventListener("keydown",u),a.removeEventListener("click",m))});(async()=>{const e=document.getElementById("quote");if(!e)return;let t;const i=localStorage.getItem("quote");if(i&&(t=JSON.parse(i),Date.now()-t.timestamp>1e3*60*60*24&&(t=null)),!t)try{t=await b(),localStorage.setItem("quote",JSON.stringify({...t,timestamp:Date.now()}))}catch{t=null}if(!t)return;const r=e.querySelector(".quote-description"),L=e.querySelector(".quote-author");r.textContent=t.quote,L.textContent=t.author,e.classList.remove("hidden")})();n.scrollUpButton.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"})});let s=0,c;function v(e){e.preventDefault(),s&&c?E(c,{rate:Number(s),email:n.ratingForm.elements.email.value,review:n.ratingForm.elements.comment.value}).then(()=>{p(),d.success({message:"Thanks for rating!"}),n.ratingForm.reset()}).catch(t=>{d.error({message:t.message})}).finally(()=>{s=0,c=null}):d.warning({message:"Please select rating"})}function k(e){const t=e.target.closest(".rating-star-container");if(t){const i=document.querySelectorAll(".js-rating-star");s=t.dataset.id,document.querySelector(".js-rating-label").textContent=Number(s).toFixed(1);for(let r=0;r<s;r++)i[r].classList.add("rating-star--active");for(let r=s;r<5;r++)i[r].classList.remove("rating-star--active")}}function q(){c=JSON.parse(document.querySelector("button[data-exercise]").dataset.exercise)._id,n.exercisePopUpBackdrop.classList.remove("is-open"),n.exercisePopUpBackdrop.style.display="none",n.ratingPopUpBackdrop.classList.add("is-open"),n.ratingPopUpContent.innerHTML=x(),P()}function p(){n.ratingPopUpBackdrop.classList.remove("is-open"),n.exercisePopUpBackdrop.style.display="flex",n.exercisePopUpBackdrop.classList.add("is-open"),w()}function B(){let e="";for(let t=0;t<5;t++)e+=`
...
//# sourceMappingURL=ratingPopup-ca6181b6.js.map
